<template>
    <div id="nav">
      <tabBar>
        <!-- 这里为什么加上  :key="index"  是因为，diff算法的要求，为了区别每一个用v-for创建出来的组件 -->
          <tabBarItem v-for="(item,index) in tb" :to_path='item[2]' :key="index"  :active_color='tb_color' >
            <div :class="item[0]"></div>
            <div class="bot">{{item[1]}}</div>
          </tabBarItem>
